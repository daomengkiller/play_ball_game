...
class online_server():
    ...
    def tcplink(self, sock, addr):
        ...
        while True:
            # sock.send(str(paddle_pos).encode('utf-8'))

            ball_pos, paddleup_pos=eval(sock.recv(1024).decode('utf-8'))
            sock.send(str(paddledown_pos).encode('utf-8'))
            # print(message_data)
            # time.sleep(0.01)
    ...
    def run(self):
        while True:
            ...
            t = threading.Thread(target=self.tcplink, args=(self.sock, self.addr))
            t.start()
...
class Paddle():
    def __init__(self, canvas, color, x_position=200, y_position=300, press_left='<KeyPress-Left>',
                 press_right='<KeyPress-Right>'):
        self.canvas = canvas
...
if __name__ == "__main__":
    ...
    my_socket = online_server()
    my_threading = threading.Thread(target=my_socket.run, args=())
    my_threading.start()

    paddle_down = Paddle(canvas, 'blue', x_position=200, y_position=500, press_left='<KeyPress-A>',
                         press_right='<KeyPress-D>')
